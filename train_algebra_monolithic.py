#!/usr/bin/env python3
"""
Monolithic Training Script for Algebra EBM Models

Trains a single unified energy-based model for all algebraic rules simultaneously.
This script implements Tasks 2.1-2.4 of the monolithic training baseline.

Usage:
    python train_algebra_monolithic.py --batch_size 2048 --timesteps 10
    python train_algebra_monolithic.py --train_steps 200000
    python train_algebra_monolithic.py --problems_per_rule 50000

Trains 1 monolithic model covering all 4 rules:
...
Model is saved to ./results/monolithic/
"""
...
def parse_args():
    ...
    # Core arguments (rule removed for monolithic training)
...
def check_hardware_compatibility(args):
    """Check hardware compatibility and provide optimization recommendations."""
    ...
    if torch.cuda.is_available():
        ...
        # GPU memory validation for batch size
        if args.batch_size >= 2048 and gpu_memory_gb < 15:
            print(f"⚠️  WARNING: Batch size {args.batch_size} may cause OOM on {gpu_memory_gb:.1f}GB GPU")
            recommended_batch = int(args.batch_size * (gpu_memory_gb / 16))
            print(f"   Recommended batch size for your GPU: --batch_size {recommended_batch}")
            print("   Alternatively, use gradient accumulation: --gradient_accumulate_every 2")
        elif args.batch_size >= 2048:
            print(f"✓ GPU memory sufficient for batch size {args.batch_size}")
        
    elif torch.backends.mps.is_available():
        print("✓ Apple Silicon MPS detected")
        print("  MPS Configuration Notes:")
        
        # MPS-specific warnings and recommendations
        if args.batch_size >= 1024:
            print(f"⚠️  WARNING: Large batch size ({args.batch_size}) on Apple Silicon may cause memory issues")
            print("   Apple Silicon unified memory has different allocation patterns than discrete GPUs")
            print("   Recommended settings for Apple Silicon:")
            print("     --batch_size 512")
            print("     --gradient_accumulate_every 4  # Effective batch size = 512 × 4 = 2048")
            print("     --fp16 false  # MPS has limited FP16 support")
            print("     --compile_backend eager  # More stable than inductor on MPS")
        
        if args.fp16:
            print("⚠️  WARNING: FP16 training on MPS may be unstable")
            print("   Consider using --fp16 false for more reliable training")
        
        if args.compile_model and args.compile_backend == 'inductor':
            print("⚠️  WARNING: Inductor backend may have issues on MPS")
            print("   Consider using --compile_backend eager for better compatibility")
        ...
        print("   • Consider training with CPU fallback if memory issues persist")
        
    else:
        print("⚠️  No GPU acceleration detected (CUDA/MPS)")
        print("   Training will use CPU, which will be significantly slower")
        print("   Strongly recommended to reduce batch size: --batch_size 64")
        
        if args.batch_size > 256:
            print(f"⚠️  WARNING: Batch size {args.batch_size} is too large for CPU training")
            print("   Recommended: --batch_size 64 --gradient_accumulate_every 32")
...
def main():
    ...
    print("Starting monolithic algebra EBM training for all rules")
    
    # Hardware compatibility check with MPS support
    check_hardware_compatibility(args)
    
    # Set up results directory
    if args.results_folder is None:
        args.results_folder = osp.join('results', 'monolithic')
    ...
    # Create combined algebra dataset for all rules
    print(f"Creating combined dataset for all rules with {args.problems_per_rule} problems per rule...")
    ...
    try:
        ...
        dataset_kwargs = {
            'split': args.split,
            'problems_per_rule': args.problems_per_rule,
        ...
        if args.use_curriculum:
            print("Warning: Curriculum learning not supported for monolithic training")
            print("Falling back to standard CombinedAlgebraDataset...")
        
        try:
            dataset = CombinedAlgebraDataset(**dataset_kwargs)
        except (ImportError, NameError) as e:
            print(f"Warning: CombinedAlgebraDataset not available ({e})")
            print("Creating placeholder dataset using first rule for testing...")
            # Fallback for testing when CombinedAlgebraDataset doesn't exist yet
            fallback_kwargs = dict(dataset_kwargs)
            fallback_kwargs['rule'] = 'distribute'
            fallback_kwargs['num_problems'] = fallback_kwargs.pop('problems_per_rule')
            dataset = AlgebraDataset(**fallback_kwargs)
    except Exception as e:
    ...
    print("Initializing AlgebraEBM for monolithic training...")
    try:
        ebm = AlgebraEBM(
            inp_dim=dataset.inp_dim,
            out_dim=dataset.out_dim,
            rule_name='monolithic'
        )
    except Exception as e:
