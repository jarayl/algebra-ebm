#!/usr/bin/env python3
"""
Script to add enhanced calibration methods to AlgebraInference class.
"""

def add_calibration_methods_to_file():
    """Add the enhanced calibration methods to the existing inference file."""
    ...
    import_additions = """import random
from collections import Counter
"""
    
    # Find the import section and add our imports
    import_pos = content.find("from collections import deque")
    if import_pos == -1:
        import_pos = content.find("import logging")
    
    if import_pos != -1:
        # Find the end of the line and add our imports
        line_end = content.find('\n', import_pos)
        content = content[:line_end+1] + import_additions + content[line_end+1:]
    
    # Calibration methods to add
    calibration_methods = '''
...
'''
    ...
    # Find the end of the AlgebraInference class to add methods
    class_start = content.find("class AlgebraInference:")
    if class_start == -1:
        raise ValueError("AlgebraInference class not found!")
    
    # Find the end of the class (before the next class or function definition)
    search_from = class_start
    class_end = -1
    
    # Look for the end of the class by finding next top-level definition
    lines = content.split('\n')
    class_line = None
    for i, line in enumerate(lines):
        if "class AlgebraInference:" in line:
            class_line = i
            break
    
    if class_line is None:
        raise ValueError("Could not find AlgebraInference class line!")
    
    # Find the last method in the class
    for i in range(class_line + 1, len(lines)):
        line = lines[i]
        
        # If we hit another class or function definition at top level, we found the end
        if line.strip() and not line.startswith(' ') and not line.startswith('\t'):
            if line.startswith('class ') or line.startswith('def '):
                class_end = i
                break
    
    # If no class_end found, add at end of file
    if class_end == -1:
        class_end = len(lines)
    
    # Insert our methods before the end of the class
    lines.insert(class_end, calibration_methods)
    ...
    print("Successfully added enhanced calibration methods to AlgebraInference class")

if __name__ == "__main__":
    add_calibration_methods_to_file()